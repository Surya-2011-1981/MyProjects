...
reset_btn.onclick=()=>{
  let confirmation=confirm("Warning Form to be reset..!");
...
photo.addEventListener('change', function(event) {
  ...
  let profile_Div=document.querySelector(".For-Profile");
  let profile_Img=document.querySelector(".profile_image");
  profile_Div.classList.remove("display-none");
  profile_Img.src="";
...
let add_Button=document.querySelector(".custom-fields");
add_Button.onclick= ()=>{
  let custom_area=document.createElement("textarea");
  let label= prompt("Enter Name of your Field");
  let custom_area_Name= document.createElement("label");
  custom_area.name=label;
  custom_area.value="";
  custom_area_Name.innerText=label+" : ";
  custom_area_Name.classList.add("custom-label");
  custom_area_Name.classList.add("Custom-"+label);
  custom_area.classList.add("custom-textarea");
  custom_area.classList.add(label+"Custom-textarea");
   add_Button.before(custom_area);
   custom_area.before(custom_area_Name);
   let form_height=form.offsetHeight + 143;
   let string_form_height=String(form_height)+"px";
  form.style.height=string_form_height;
}
